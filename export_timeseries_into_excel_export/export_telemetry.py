...
def get_timeseries(base_url, jwt, device_id, keys, start_ts, end_ts):
    headers = {"X-Authorization": f"Bearer {jwt}"}
    params = {
        "keys": ",".join(keys),
        "startTs": start_ts,
...
def main():
    ...
    base_url = prompt_with_retry("Enter Base URL [Default: http://localhost:8080]: ",
    ...
    for dev_name in device_names:
        ...
        ws.append(["Timestamp (IST)", "Timestamp (UTC ms)"] + keys_to_use)

        timestamps = sorted({item['ts'] for k in data for item in data[k]})

        for ts in timestamps:
            row = [
                datetime.fromtimestamp(ts / 1000, IST).strftime('%Y-%m-%d %H:%M:%S'),
                ts  # raw UTC ms
            ]
